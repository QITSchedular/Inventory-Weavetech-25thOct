...
function LogStatus() {
  ...
  //2. Send Filter Log Status
  const handleDisplayAddITRLogStatus = async () => {
    const fromDateOne1 = formatDate(fromDateOne, "yyyy-MM-dd");
    const toDateTwo2 = formatDate(toDateTwo, "yyyy-MM-dd");
    // Send Log Data API
    if (fromDateOne === "" && toDateTwo === "") {
      ...
      console.log("first57678789", postLogData);
      try {
        let response = await axios.post(
          `${BACKEND_URL}/api/Logs/GetTransactionLog`,
          postLogData
        );
        ...
        <SoftBox py={3} mb={0} textAlign="center">
          <SoftBox ml={2} mr={2} mb={0} mt={0}>
            ...
            <SoftBox container spacing={1} mt={5}>
              <SoftButton
                onClick={handleDisplayAddITRLogStatus}
                variant="contained"
                color="info"
                style={{
