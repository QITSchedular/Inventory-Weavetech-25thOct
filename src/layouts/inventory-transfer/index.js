...
const InventoryTransferRequest = () => {
  ...
  const [isActiveOverLay2, setIsActiveOverLay2] = useState(false);
  ...
  // DataGrid
  function renderPrimaryTab(props) {
    return <PrimaryTab data={props.data.data} />;
  }
  function renderSecondaryTab(props) {
    return <SecondaryTab data={props.data.data} />;
  }
      ...
      <React.Fragment>
        <TabPanel>
          <Item
            ...
            render={renderPrimaryTab}
            data={props}
          />
          <Item
            title="Item wise Quantity Warehouse"
            render={renderSecondaryTab}
            data={props}
          />
        </TabPanel>
      </React.Fragment>
  ...
  function gridBoxDisplayExpr20(item) {
    return item && `${item.whsCode} -- ${item.locationId} `;
  }
  ...
  function PrimaryTab(props) {
    ...
    const handleRefresh2 = async () => {
      ...
      newMasterListInLayoutEffect3.map(async (element, index) => {
        try {
          ...
          let qtyInStock = await response.data[0].qtyInStock;
          let id = element.id;
    ...
    const handleContinue5 = async () => {
      // setProgressLoading(true);
      console.log("object", popupItemCode);
      let whsAPILength = whsAPIList1.length;
      var updatedMasterListWhsArray3 = [];
      whsAPIList1.map(async (element, index) => {
        try {
          ...
          setTimeout(() => {
            if (whsAPILength === updatedMasterListWhsArray3.length) {
              updatedMasterListWhsArray3.sort(function (x, y) {
                return x.id - y.id;
              });
              console.log(
                "updatedMasterListWhsArray3",
                updatedMasterListWhsArray3
              );
              console.log("object", headerList);
          ...
          <Card>
            <SoftBox ml={1} mr={1} mb={5} mt={0}>
              <div>
                <div>
                  {(() => {
                    if (newMasterListAfterLayout[0] === undefined) {
                      return (
                        <DataGrid
                        ...
                        >
                          ...
                          <Toolbar>
                            ...
                            <Item location="after">
                              <Button
                                ...
                                icon="refresh"
                                text="Update From Warehouse"
                              />
                            </Item>
                            <Item
                              name="Item Wise Warehouse Details"
                              showText="always"
                            />
                            <Item location="before">
                              <Button
                                onClick={showPopup}
                                //icon=""
                                text="Item wise Warehouse Details"
                              />
                            </Item>
                          </Toolbar>
                          <FilterRow visible={true} />
                          ...
                          <Column
                            dataField="id"
                            caption="Item-wise Whs"
                            alignment="center"
                            visible={false}
                          >
                            <Lookup
                              dataSource={whsAPIList2}
                              valueExpr="id"
                              displayExpr={gridBoxDisplayExpr20}
                              //onOptionChanged={onGridBoxOpened18}
                            />
                          </Column>
                          <Column
                ...
                <div>
                  ...
                  >
                    <ScrollView width="100%" height="100%">
                      <SoftBox textAlign="center" mt={4} mb={3} ml={1} mr={1}>
                        <DropDownBox
        ...
        <Card>
          ...
          <Card>
            <SoftBox ml={1} mr={1} mb={5} mt={0}>
              ...
              <SoftBox style={{ display: "flex" }} mt={4}></SoftBox>
            </SoftBox>
          </Card>
        </Card>
      </React.Fragment>
