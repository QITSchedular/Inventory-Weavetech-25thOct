...
function LogStatus() {
  ...
  const onExporting = React.useCallback((e) => {
    if (e.format === "xlsx") {
      ...
      const worksheet = workbook.addWorksheet("APILogStatus");
      exportDataGrid({
  ...
  //3. Send Filter Log Status
  const handleDisplayAddAPILogStatus = async () => {
    if (fromDateOne === "" && toDateTwo === "") {
      ...
      try {
        let response = await axios.post(
          `${BACKEND_URL}/api/Logs/GetAPILog`,
          postLogData
        );
        ...
        <SoftBox py={3} mb={0} textAlign="center">
          <SoftBox ml={2} mr={2} mb={0} mt={0}>
            ...
            <SoftBox container spacing={1} mt={5}>
              <SoftButton
                onClick={handleDisplayAddAPILogStatus}
                variant="contained"
                color="info"
                style={{
